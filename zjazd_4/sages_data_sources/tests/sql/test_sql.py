...
class TestSql:
    ...
    def test_load_passengers_by_sex_and_age(self, load_titanic_data):
        ...
        #df = run_query("select Sex from passengers")
    ...
    def test_find_survivors(self, load_titanic_data):
        ...
        #df = run_query("select survived from passengers")
        df = run_query("""select Name from passengers p
                       JOIN survivors s
    ...
    def test_count_by_embarked(self, load_titanic_data):
        ...
        df = run_query("select Embarked as embarked, count(*) as number_of_passengers from tickets where Embarked is not null group by Embarked")
    ...
    def test_find_max_min_age_by_sex_and_survived(self, load_titanic_data):
        ...
        """
        df = run_query("""
        select Sex, Survived, MAX(Age) as Oldest, MIN(Age) as Youngest from passengers p
        JOIN survivors s
        ON p.PassengerId = s.PassengerId
